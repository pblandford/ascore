...
internal fun createMxmlScore(score: Score): MxmlScorePartwise? {
  ...
  val parts = score.parts.withIndex().mapNotNull { part ->
    createPart(
      part.value, partList.scoreParts.toList()[part.index], part.index + 1, score,
      repeatBarQuery
    )
  }
...
private fun createPartList(score: Score): MxmlPartList {
  val scoreParts = score.parts.withIndex().mapNotNull {
    createScorePart(it.value, it.index + 1)
  }
