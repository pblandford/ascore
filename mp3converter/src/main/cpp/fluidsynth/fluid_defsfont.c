...
fluid_sfont_t* fluid_defsfloader_load(fluid_sfloader_t* loader, const char* filename)
{
  ...
  FLUID_LOG(FLUID_ERR, "Is this even the right place?");

  defsfont = new_fluid_defsfont();

  if (defsfont == NULL) {
    FLUID_LOG(FLUID_ERR, "Unlikely");
    return NULL;
  }

  if (fluid_defsfont_load(defsfont, filename) == FLUID_FAILED) {
    FLUID_LOG(FLUID_ERR, "Just generally failed");
...
sfload_file (const char * fname)
{
  ...
  if (!(fd = fopen (fname, "rb")))
    {
      FLUID_LOG (FLUID_ERR, _("Unable to open file \"%s\" %s"), fname, strerror(errno));
      return (NULL);
    }
  ...
  if (!(sf = FLUID_NEW (SFData)))
	...
	FLUID_LOG(FLUID_DEBUG, "Got here at least");
