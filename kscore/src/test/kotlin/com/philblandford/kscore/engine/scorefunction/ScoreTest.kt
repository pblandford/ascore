...
open class ScoreTest {
  ...
  protected val sc = ScoreContainer(drawableFactory)
  ...
  fun SAE(
    ...
    endAddress: EventAddress? = null, destination: ScoreLevelType? = null
  ) {
  ...
  fun SDE(
    ...
    params: ParamMap = paramMapOf(), endAddress: EventAddress? = null
  ) {
    sc.deleteEvent(eventType, eventAddress, endAddress, params)
  }
  ...
  fun SSP(
    eventType: EventType, eventParam: EventParam, value: Any?, eventAddress: EventAddress = eZero(),
  ...
  fun BeamMap.getBeamStrings(): List<String> {
    return values.map { it.members.map {
  ...
  protected fun SCORE() = sc.currentScore.value!!
