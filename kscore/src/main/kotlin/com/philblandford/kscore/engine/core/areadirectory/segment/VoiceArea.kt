...
fun DrawableFactory.restArea(event: Event): VoiceArea? {
  ...
  return restKeys[event.duration().undot()]?.let { desc ->
    getDrawableArea(ImageArgs(desc.key, INT_WILD, (desc.height * BLOCK_HEIGHT * mult).toInt()))?.let {
      ...
      repeat(event.duration().numDots()) {
        getDrawableArea(DotArgs(DOT_WIDTH, DOT_WIDTH))?.let { dot ->
          base = base.addRight(dot.copy(tag = "Dot"), DOT_WIDTH, (BLOCK_HEIGHT * 2.3f).toInt())
        }
      }
      VoiceArea(
        base, VoiceGeography(
