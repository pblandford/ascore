...
object TupletSubAdder : NewSubAdder {
  ...
  override fun <T> setParam(
    ...
    return if (param == EventParam.HARD_START) {
      (value as? Coord).ifNullFail("Hard start is not coord!").then { coord ->
  ...
  override fun addEventRange(
    ...
    var events = score.getEvents(EventType.DURATION, eventAddress, endAddress)?.toList() ?: listOf()
  ...
  private fun Event.endIsLastMember(eventAddress: EventAddress, endAddress: EventAddress): Boolean {
  ...
  private fun Score.createTuplet(params: ParamMap, eventAddress: EventAddress): AnyResult<Tuplet> {
    ...
    return params.g<Int>(EventParam.NUMERATOR)?.let { numerator ->
      params.g<Duration>(EventParam.DURATION)?.let { duration ->
        ...
        val duration = getParam<Duration>(EventType.DURATION, EventParam.DURATION, eventAddress)
