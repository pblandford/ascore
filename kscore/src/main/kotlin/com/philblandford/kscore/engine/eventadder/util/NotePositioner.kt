...
fun setAllPositions(
    chordEvent: Event, clef: ClefType, voice: Int? = null, octaveShift: Int = 0,
    grace: Boolean = false
): Event {
    return chord(chordEvent)?.let { chord ->
        val yPositions =
            chord.notes.map { getYPosition(it.pitch, clef, octaveShift, it.position) ?: Coord() }
                .toList().sortedBy { it.y }
        val chordUpstem = chordEvent.params.g<Modifiable<Boolean>>(EventParam.IS_UPSTEM)
            ?.let { if (it.modified) it.value else null }
        val up = chordUpstem ?: getStemDirection(yPositions, voice, grace)
...
fun setXPositions(chordEvent: Event): Event {
    return chord(chordEvent)?.let { chord ->
        val positions = getXPositions(
            chord.notes.map { it.position }.sortedBy { it.y },
            chord.upstem()
        ).toList()
...
fun getXPositions(yPositions: Iterable<Coord>, up: Boolean): List<Coord> {
    ...
    val clusterPositions = clusters.flatMap { setClusterPositions(it, up) }
...
fun setStemDirection(chord: Event, numVoices: Int, voiceNum: Int, grace: Boolean): Event {
    val up = if (numVoices > 1) {
        ...
        getStemDirection(positions, null, grace)
    }
    return chord.setModValue(EventParam.IS_UPSTEM, up)
}

fun getStemDirection(
...
private fun setClusterPositions(cluster: Cluster, up: Boolean): Iterable<Coord> {
