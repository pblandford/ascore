...
private fun DurationMap.removeOutsizeRests(): DurationMap {
    var filtered = map.toList().windowed(2).fold(map) { m, (one, two) ->
    ...
    filtered = filtered.filter { it.value.duration.denominator.isPower2() }.toSortedMap()
...
private fun DurationMap.divideMap(): List<DurationMap> {
    ...
    splitDmList = terminated.windowed(2).map { (one, two) ->
        ...
        events =
            events.map { it.key - (one.offset - this.offset) to it.value }.toMap().toSortedMap()
        one.copy(map = events)
    }
    return splitDmList
}
...
private fun List<DurationMap>.merge(timeSignature: TimeSignature): DurationMap {
    val offsetsRestored = map { dm ->
        val events =
            dm.map.filterNot { it.value.type == DurationType.NONE }
                .map { it.key + dm.offset to it.value }
                .toMap().toSortedMap()
        dm.copy(map = events)
    }
