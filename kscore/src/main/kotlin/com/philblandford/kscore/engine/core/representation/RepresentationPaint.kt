...
private fun getLastSegment(system: Area, staveId: StaveId): EventAddress? {
    return system.findByTag("Part").toList()
        .find { it.first.eventAddress.staveId.main == staveId.main }?.let { (_, part) ->
            part.findByTag("Stave").toList()
                .find { it.first.eventAddress.staveId.sub == staveId.sub }
                ?.let { (_, stave) ->
...
internal fun Area.paintBorders(
    ...
    return childMap.toList().fold(this) { thisArea, (key, child) ->
        val painted = if (condition(key, child)) {
            val rectArea =
                drawableFactory.getDrawableArea(RectArgs(child.width, child.height - child.yMargin, false, red()))
            val yMarginArea = drawableFactory.getDrawableArea(
                RectArgs(
                    child.width,
                    child.yMargin,
                    false,
            ...
            rectArea?.let {
                yMarginArea?.let {
                    thisArea.addArea(yMarginArea, Coord(key.coord.x, key.coord.y - child.yMargin))
                        .addArea(rectArea, key.coord)
                }
            } ?: thisArea
        } else {
            thisArea
