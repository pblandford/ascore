...
fun <L : Failure, R> ASResult<L, R>.ignoreFailure(originalThing: R): ASResult<L, R> =
  when (this) {
    is Left ->
      Warning(
        l, originalThing, messages.plus(Exception("Ignoring failure ${l.message}", l.exception))
      )
    else -> this
  }

fun <L : Failure,  R> ASResult<L, R>.failureIsNoop(originalThing: R, ifOkDo:(R)->ASResult<L, R>): ASResult<L, R> =
  when (this) {
    is Left ->
      Warning(l, originalThing, messages.plus(Exception("Ignoring failure ${l.message}", l.exception)))
...
fun <R1, R2> R1?.ifNullError(
  ...
  return this?.let { Right(it).then(f) } ?: asError(message)
}
