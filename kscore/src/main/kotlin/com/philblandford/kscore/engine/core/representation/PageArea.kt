...
internal fun DrawableFactory.createPages(
    ...
    val breaks =
        scoreQuery.getEvents(EventType.BREAK)?.filter { it.value.subType == BreakType.PAGE }
...
internal fun DrawableFactory.pageArea(
    ...
    area = addFooter(area, scoreQuery, layoutDescriptor, num)
...
private fun DrawableFactory.addFooter(
    ...
    val footer = pageFooterArea(num, scoreQuery, layoutDescriptor)
...
private fun DrawableFactory.addSystems(
    ...
    val area = systemPositions.toList().fold(pageArea) { pa, (_, sp) ->
        ...
        pa.addArea(
            sysArea, Coord(layoutDescriptor.leftMargin, sp.pos),
...
private fun getSystemPositions(
    ...
    run loop@{
        geographies.forEach { sysYGeog ->
            ...
            systemPositions.add(
                sysYGeog.xGeog.startBar to SystemPosition(
                    totalHeight + sysYGeog.yMargin,
                    sysYGeog
                )
            )
    ...
    val spacedSystems = if (extraY < available / 25) {
    ...
    return getSystemPosition(eventAddress, this)?.let { sysPos ->
        sysPos.second.systemYGeography.xGeog.barPositions[eventAddress.barNum]?.let { barPos ->
            val startBar =
                layoutDescriptor.leftMargin + sysPos.second.systemYGeography.xGeog.startMain + barPos.pos
            val y = sysPos.second.pos
            barPos.geog.original.slicePositionsList.lastOrNull { it.first.offset <= eventAddress.offset }
                ?.let { (_, slicePos) ->
